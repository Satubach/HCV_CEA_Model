...
{with(as.list(c(state, parameters)), {
 ...
 treated <- (RNA_tests_I * treat.rate) + (RNA_tests_C * treat.rate)       
  ...
  dS <- ent.rate - 
    effective.FOI * S +                                 
    (RNA_tests_I * treat.rate * SVR.rate) -            
  ...
  SSP_Imp <- ifelse(time == 1 & SSP.rate > 0, SSP_Implementation_Cost, 0)
...
base_initial <- c(S = 11763, I = 14368, C = 9)
...
owsa_list <- list(
    ...
    "HCV/PWID prevalence (initial_state)" = list (
      values = list (
        low = c(S = 15500, I = 10632, C = 6),
        high = c(S = 8364, I = 17764, C = 11)
      ),
    type = "initial"
  ),
   
  "FOI.rate" = list(values = c(low = 0.32, high = 0.73), type = "parameter"),
  "Mu1.rate" = list(values = c(low = 0.04, high = 0.14), type = "parameter"),
  "treat_Cost_Param" = list(values = c(low = 17101, high = 26083), type = "parameter"),
...
for (scenario_name in names(intervention_list)) {
  ...
  # Runs the SIS HCV model for interventions using ode solver from deSolve and calculates incremental costs and DALYs averted from baseline.
...
owsa_ICERs <- owsa_icers %>%
  ...
  write.csv(owsa_icers, "C:\\Users\\stubach\\OneDrive - State of Kansas, OITS\\Documents\\Dissertation\\DATA\\New Analysis\\Final Analysis\\HCV_OWSA_results_Final.csv", 
          row.names = FALSE) 
 
  write.csv(owsa_ICERs, "C:\\Users\\stubach\\OneDrive - State of Kansas, OITS\\Documents\\Dissertation\\DATA\\New Analysis\\Final Analysis\\HCV_OWSA_plot_Final.csv", 
            row.names = FALSE)
